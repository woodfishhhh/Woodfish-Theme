...
- 重做 Dracula 语法渐变：从六类主要语法色的 Dracula 原色自动计算多段 OKLCH 渐变，固定原色中点，组合 `ΔL ±0.060` 与色相 `±24°`，并加入色域映射与短 token 渐变宽度约束
- 语法渐变与文字发光改为按主题颜色编译 `mtk` 选择器，并在编辑器前景色配置变化时重新同步，避免依赖固定编号
